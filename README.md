# Ciao, sono Gianluca

Benvenuto/a sul mio profilo GitHub!
2803
![Screenshot 2024-10-29 103233](./images/terraquakeapi.png)

# Progetto importante

## TerraQuake API – Dati Sismici in Tempo Reale
- TerraQuake API è un servizio API avanzato che fornisce dati sismici in tempo reale da fonti ufficiali e affidabili. È pensata per sviluppatori, ricercatori, istituzioni e app che desiderano integrare informazioni sui terremoti in maniera semplice, precisa e aggiornata.

### Caratteristiche principali
- Dati sismici in tempo reale: accesso immediato a informazioni su terremoti recenti, inclusi magnitudo, epicentro, profondità, ora e autorità di rilevamento.
- Filtri avanzati: possibilità di interrogare l’API per magnitudo minima, aree geografiche specifiche, intervalli di tempo e altre variabili.
- Formati multipli: output in JSON, facilmente integrabile in applicazioni web, mobile o software di analisi.
- Esempi di utilizzo: guide pratiche per cURL, Python, Axios e React, facilitando l’integrazione nelle app.
- Affidabilità e aggiornamenti continui: i dati sono aggiornati costantemente da fonti geologiche ufficiali, garantendo precisione e tempestività.

- Visita il progetto online [https://terraquakeapi.com]
- Clona il progetto [https://github.com/nagcas/TerraQuakeApi]

### Obiettivo del progetto
TerraQuake API mira a diventare uno strumento chiave per chiunque necessiti di dati sismici affidabili: da app di allerta in tempo reale come SafeQuake Alert, a dashboard di monitoraggio per istituzioni, fino a progetti di ricerca scientifica. Grazie alla semplicità di integrazione e alla completezza dei dati, gli sviluppatori possono creare soluzioni innovative per la prevenzione e la sicurezza pubblica.

## 🌱 Competenze

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB) ![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Express](https://img.shields.io/badge/Express-000000?tyle=for-the-badge&logo=express&logoColor=white) ![Bootstrap](https://img.shields.io/badge/Bootstrap-7952B3?style=for-the-badge&logo=bootstrap&logoColor=white)
![AOS](https://img.shields.io/badge/AOS-000000?style=for-the-badge&logo=aos&logoColor=white) ![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=white)
![Socket.io](https://img.shields.io/badge/Socket.io-010101?style=for-the-badge&logo=socketdotio&logoColor=white) ![Telegraf](https://img.shields.io/badge/Telegraf-000000?style=for-the-badge&logo=telegram&logoColor=white)
![Leaflet](https://img.shields.io/badge/Leaflet-199900?style=for-the-badge&logo=leaflet&logoColor=white) ![Cloudinary](https://img.shields.io/badge/Cloudinary-3448C5?style=for-the-ge&logo=cloudinary&logoColor=white)
![i18next](https://img.shields.io/badge/i18next-26A69A?style=for-the-badge&logo=i18next&logoColor=white) ![React-i18next](https://img.shields.io/badge/React--i18next-61DAFB?style=for-the-badge&logo=react&logoColor=white)
![React Hot Toast](https://img.shields.io/badge/React--Hot--Toast-61DAFB?style=for-the-badge&logo=react&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)
![Django](https://img.shields.io/badge/Django-092E20?style=for-the-badge&logo=django&logoColor=white)
![SQLite](https://img.shields.io/badge/SQLite-07405E?style=for-the-badge&logo=sqlite&logoColor=white)
![SQLAlchemy](https://img.shields.io/badge/SQLAlchemy-D71F00?style=for-the-badge&logo=sqlalchemy&logoColor=white)
![Discord.py](https://img.shields.io/badge/Discord.py-5865F2?style=for-the-badge&logo=discord&logoColor=white)

## 📫 Come contattarmi:
- Email: studio.nagcas@outlook.it 
- LinkedIn: (https://www.linkedin.com/in/gianluca-chiaravalloti-5694081a2/)
- Portfolio: (https://portfolio-gianluca-phi.vercel.app/)
